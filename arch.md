...
> (`miniaudio.h`, `tsf.h`, `ogg.h`, `codec.h`, `vorbis.c`, and the `opus/`
> decoder stack: libogg + libopus + libopusfile) is excluded from the
...
TinySoundFont, OGG/Vorbis, and the Opus decoder stack under `opus/`). This buys four properties:
...
keyboard audition set. §5.7 documents track duplication (duplicate-select mode)
and the Buffer effect's negative-rate reverse playback.
...
### 5.7 Track Duplication (duplicate-select mode) & the Buffer effect's negative rate

**Track Duplication — duplicate-select mode.** The bottom-dock **DUP** button
does not duplicate immediately. It toggles `g_Seq.isDupSelectMode`
(`types.h`), arming a select mode: `render_ui` (`ui.h`) greys the whole track
area out via `draw_alpha_box` and pops the hovered track row back into color
using its theme `waveColor`. `WM_MOUSEMOVE` (`events.h`) invalidates while the
mode is armed so the highlight follows the cursor. A `WM_LBUTTONDOWN` over any
track row sets `lastClickedTrack` to that row, calls `duplicate_track_action`,
and exits the mode (clicks outside a track row leave it armed).

`duplicate_track_action` (`actions.h`) clones the source track: every per-track
array (including `trackTriggerProb`/`trackRngState`), the FX chain via
`fx_chain_to_snapshot` (fresh per-instance state, no shared heap pointers), the
per-track granular engine with owned-sample pointers dropped, and all of the
source's clips (per-clip granular state re-pointed; synth clips re-initialized
**off the lock** — see §3.3/§6.6). It appends the copy at the end, then calls
`reorder_track(trackCount-1, src+1)` so the duplicate lands **directly below**
the source and the tracks below shift down by one. This is why `reorder_track`
must swap **every** per-track array: it already handled the EQ/filter/theme/
FX/granular rows, and it now also carries `trackTriggerProb` and
`trackRngState` (a latent gap fixed here) so a moved/duplicated track keeps its
trigger-probability and RNG state.

**Buffer effect — negative rate (reverse).** The Buffer effect's Rate knob
(`kFxBuffParams[2]`, `fx.h`) spans **-2.00x to 2.00x** with default **0.00x**
at the middle; the UI derives range/default/drag/wheel entirely from the
`FxParamDef`, so no UI code changed. In `fx_buff_process` the rate clamp is now
sign-preserving with a ±0.01 magnitude floor, the read-position wrap is
bidirectional (`readPos < 0` wraps up by `size`), and the linear-interpolation
neighbor is direction-aware (`ip-1` when reversing) so negative rates play the
internal loop buffer in reverse. The Ramp retrigger snaps `readPos` to a fixed
point and is direction-agnostic, so it works for reverse too. Serialization is
unaffected: FX params are stored as raw floats, the range lives only in the
compiled table, so old files with positive rates load unchanged and the new
default only affects freshly inserted instances.

### 5.8 External Sidechain Routing & the Reserved Slot-0 Compressor

**Model.** `SequencerState.trackSidechainSource[MAX_TRACKS]` (`types.h`) holds,
per track, `-1` (internal/self detection) or the index of the track whose
pre-FX signal ducks it. It is initialized to `-1` at startup (`main.c`), on
reset (`reset_to_init_state`, `dialogs.h`), and on project load (`project.h`),
and is captured/restored in undo snapshots (`state.h`).

**DSP.** The sidechain feed lives **per `FxInstance`** (`scFeedL/scFeedR/
scActive`, `fx.h`), not in globals, so live playback and export (which render
from separate snapshots and may run concurrently — §2) never share state. In
`render_frames` (`audio.h`) the pre-FX track audio is snapshotted each frame;
the routed track's slot-0 compressor gets `scActive` + the source track's feed
before `fx_chain_process` runs, and `fx_compressor_process` substitutes the
feed into its peak detector when active (the VCA still ducks the track's own
signal). `RenderContext`/`ExportSnapshot` carry the routing table.

**UI.** `set_track_sidechain_source` (`dialogs.h`) routes a track and, when a
source is set, inserts a compressor at FX slot 0 (pinned) with ducking
defaults (‑20 dB, 6:1, 2 ms attack, 80 ms release, 4 dB knee, 0 dB makeup);
unlinking removes that reserved compressor. The slot-0 compressor cannot be
removed through the FX rack (drag/right-click/DEL/Clear-all are blocked while
linked); unlinking is done only from the track's `Sidechain Input` context
menu (`None (Internal)` or the `Unlink Sidechain` toggle). The reserved slot
shows its source in its title (`Compressor (Sidechain: Tn)`), and the track
header shows a `[SC -> Tn]` badge (cache-invalidated on change).

**Serialization.** `CSQ_TRACKMIX_VERSION` bumped to 2; `CSQTrackMix` gained
`sidechainSource`. Save writes it; load reads the v1 prefix then the v2 field
(defaulting to `-1` for old files).
...
1. **Timeline cache** (`g_cacheDC`/`g_cacheBmp`, `ui.h:2328`) — the static
   ...
   Note: the scroll-shift path is disabled whenever `g_Seq.clipCount > 0`
   (`update_timeline_cache`, `ui.h:2816`) — `ScrollDC` moves all pixels
   including clip titles, and re-drawing only the exposed strip leaves the
   shifted-out titles on the cache, so small scrolls (e.g. the [FROM START]
   snap to beat 0) duplicated titles. With any clips present the cache is
   always fully rebuilt, clearing stale titles.
