...
typedef struct {
    ...
    FxChain*            trackFx;           
    const int8_t*       trackSidechainSource;  // -1 = internal, else source track index
...
typedef struct {
    ...
    int8_t              trackSidechainSource[MAX_TRACKS];  // -1 = internal, else source track index
    ...
    const RenderContext* ctx)
{
    ...
    for (ma_uint32 f = 0; f < frames; ++f) {
        ...
        // Snapshot the pre-FX raw track audio for external sidechain feeds.
        // The compressor on a routed track's slot 0 reads this frame's source
        // track signal (before any FX) so it ducks cleanly regardless of where
        // the source sits in the track order.
        float scFeedL[MAX_TRACKS], scFeedR[MAX_TRACKS];
        for (int tt = 0; tt < nTracks; ++tt) { scFeedL[tt] = trackL[tt]; scFeedR[tt] = trackR[tt]; }
         ...
         
        {
            ...
            while (lo | hi) {
                ...
                if (ctx->trackFx && ctx->trackFx[t].count > 0) {
                    // Route the external sidechain feed into the slot-0
                    // compressor (the reserved slot for a sidechained track).
                    int scSrc = ctx->trackSidechainSource ? ctx->trackSidechainSource[t] : -1;
                    bool hasSc = (scSrc >= 0 && scSrc < nTracks && scSrc != t);
                    ctx->trackFx[t].slots[0].scActive = hasSc;
                    if (hasSc) {
                        ctx->trackFx[t].slots[0].scFeedL = scFeedL[scSrc];
                        ctx->trackFx[t].slots[0].scFeedR = scFeedR[scSrc];
                    }
                    fx_chain_process(&ctx->trackFx[t], &L, &R);
                    ctx->trackFx[t].slots[0].scActive = false;
                }
...
static inline void audio_callback(ma_device* pDevice, void* pOutput, const void* pInput, ma_uint32 frameCount) {
    ...
    while (framesDone < frameCount) {
        ...
        RenderContext liveCtx = {
            ...
            .trackFx       = g_TrackFx,
            .trackSidechainSource = g_Seq.trackSidechainSource,
            .limiter       = s_limiter,
            .isLofi        = g_Seq.isLofi,
...
static DWORD WINAPI ExportTimelineThreadProc(LPVOID lpParam) {
    ...
    for (int t = 0; t < snap->trackCount; ++t) {
        ...
        snap->trackSidechainSource[t] = g_Seq.trackSidechainSource[t];
    }
    ...
    RenderContext exportCtx = {
        ...
        .trackSidechainSource = snap->trackSidechainSource,
        .limiter       = &e_limiter,
        .isLofi        = snap->isLofi,
