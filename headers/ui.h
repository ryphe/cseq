...
static inline DWORD compute_timeline_param_hash(void) {
    ...
    for (int t = 0; t < g_Seq.trackCount && t < MAX_TRACKS; ++t) {
        ...
        hsh = hash_dword(hsh, (DWORD)(g_Seq.trackSidechainSource[t] + 1));
...
static inline void update_timeline_cache(HDC hdc, int w, int h, const RECT *win) {
    ...
    if (!win && !g_timelineDirty && g_cacheBmp && !is_timeline_dirty(w, h) &&
        !viewport_has_invalid_bars(w) && g_Seq.clipCount == 0) {
    ...
    for (int t = 0; t < g_Seq.trackCount && t < MAX_TRACKS; ++t) {
        ...
        if (g_Seq.trackSidechainSource[t] >= 0) {
            char scBuf[24];
            snprintf(scBuf, sizeof(scBuf), "[SC -> T%d]", g_Seq.trackSidechainSource[t] + 1);
            RECT scRect = { scale_x(10), curBadgeY, get_track_header_width() - scale_x(10), curBadgeY + badgeH };
            FillRect(g_cacheDC, &scRect, cached_solid_brush(RGB(30, 50, 60)));
            SetTextColor(g_cacheDC, RGB(80, 220, 240));
            DrawTextA(g_cacheDC, scBuf, -1, &scRect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
            curBadgeY += badgeStep;
        }

        if (!isMuted && !isGran && !isSolo && g_Seq.trackSidechainSource[t] < 0) {
            char volBuf[16];
...
static inline void render_ui(HDC hdc, const RECT *clientRect) {
    ...
    // Duplicate-select mode: grey the whole track area out and pop the hovered
    // track back into color, so the user can see exactly which track a click
    // will duplicate.
    if (g_Seq.isDupSelectMode) {
        int viewTop = get_header_height();
        int viewBottom = h - get_bottom_dock_height();
        int dupTrackAreaEndY = viewTop - g_Seq.scrollY + g_Seq.trackCount * get_track_height();
        int bodyBottom = (dupTrackAreaEndY < viewBottom) ? dupTrackAreaEndY : viewBottom;
        if (bodyBottom > viewTop) {
            // Dim everything (track headers + canvas) so tracks read greyed-out.
            draw_alpha_box(memDC, 0, viewTop, w, bodyBottom - viewTop, RGB(0, 0, 0), 110, RGB(0, 0, 0));

            // Highlight the hovered track row in its theme color.
            int hovTrack = -1;
            if (g_Seq.mouseY >= viewTop && g_Seq.mouseY < viewBottom && g_Seq.mouseY < bodyBottom) {
                int t = (g_Seq.mouseY - viewTop + g_Seq.scrollY) / get_track_height();
                if (t >= 0 && t < g_Seq.trackCount) hovTrack = t;
            }
            if (hovTrack >= 0) {
                int ty = viewTop - g_Seq.scrollY + hovTrack * get_track_height();
                COLORREF hl = g_Seq.trackThemes[hovTrack % MAX_TRACKS].waveColor;
                draw_alpha_box(memDC, 0, ty, w, get_track_height(), hl, 70, hl);
            }
        }
    }
    ...
    bool isDupHover = (g_Seq.mouseX >= btnStartX + btnW + btnGap && g_Seq.mouseX <= btnStartX + btnW * 2 + btnGap &&
                       g_Seq.mouseY >= btnY && g_Seq.mouseY <= btnY + btnH);
    bool isDelHover = (g_Seq.mouseX >= btnStartX + btnW * 2 + btnGap * 2 && g_Seq.mouseX <= btnStartX + totalBtnsW &&
                       g_Seq.mouseY >= btnY && g_Seq.mouseY <= btnY + btnH);
    ...
    RECT dupBtn = {btnStartX + btnW + btnGap, btnY, btnStartX + btnW * 2 + btnGap, btnY + btnH};
    bool canDup = (g_Seq.trackCount < MAX_TRACKS);
    bool dupArmed = g_Seq.isDupSelectMode && canDup;
    HBRUSH dupBrush = CreateSolidBrush(dupArmed ? RGB(26, 40, 62) : (isDupHover && canDup ? RGB(28, 40, 52) : RGB(28, 34, 42)));
    HPEN dupPen = CreatePen(PS_SOLID, 1, dupArmed ? RGB(150, 215, 255) : (isDupHover && canDup ? RGB(110, 180, 235) : RGB(70, 110, 150)));
    HGDIOBJ oldDupB = SelectObject(memDC, dupBrush);
    HGDIOBJ oldDupP = SelectObject(memDC, dupPen);
    RoundRect(memDC, dupBtn.left, dupBtn.top, dupBtn.right, dupBtn.bottom, 4, 4);
    SetTextColor(memDC, canDup ? (dupArmed ? RGB(190, 230, 255) : (isDupHover ? RGB(170, 215, 255) : RGB(120, 175, 220))) : RGB(75, 80, 90));
    {
        HFONT oldBtnFont = (HFONT)SelectObject(memDC, get_ui_small_font());
        DrawTextA(memDC, "DUP", -1, &dupBtn, DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
        SelectObject(memDC, oldBtnFont);
    }
    SelectObject(memDC, oldDupP);
    SelectObject(memDC, oldDupB);
    DeleteObject(dupPen);
    DeleteObject(dupBrush);

    RECT delBtn = {btnStartX + btnW * 2 + btnGap * 2, btnY, btnStartX + totalBtnsW, btnY + btnH};
