...
static LRESULT CALLBACK FxRackWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT: {
        ...
        for (int i = 0; i < FX_MAX_SLOTS; ++i) {
            ...
            if (filled) {
                ...
                bool isSidechainSlot = (i == 0 && g_fxTrack >= 0 && g_fxTrack < MAX_TRACKS &&
                                        g_Seq.trackSidechainSource[g_fxTrack] >= 0 &&
                                        type == FX_TYPE_COMPRESSOR);
                int typeCount = 0;
                for (int k = 0; k < chain->count; ++k) {
                    if (chain->slots[k].desc && chain->slots[k].desc->type == type) typeCount++;
                }
                // The sidechain source is shown in the reserved slot's title.
                char scTag[48];
                if (isSidechainSlot)
                    snprintf(scTag, sizeof(scTag), " (Sidechain: T%d)",
                             g_Seq.trackSidechainSource[g_fxTrack] + 1);
                else
                    scTag[0] = '\0';
                if (typeCount > 1) {
                    int thisNum = 0;
                    ...
                    snprintf(rowTxt, sizeof(rowTxt), "%d.  %s %d%s", i + 1, nm, thisNum, scTag);
                } else {
                    snprintf(rowTxt, sizeof(rowTxt), "%d.  %s%s", i + 1, nm, scTag);
                }
    ...
    case WM_LBUTTONDOWN: {
         ...
         
        {
            ...
            if (mx >= btn.left && mx <= btn.right && my >= btn.top && my <= btn.bottom) {
                if (chain->count > 0) {
                    // The pinned sidechain compressor can't be removed through
                    // the FX rack; unlink it via the track's Sidechain Input
                    // context menu first.
                    if (g_fxTrack >= 0 && g_fxTrack < MAX_TRACKS &&
                        g_Seq.trackSidechainSource[g_fxTrack] >= 0) {
                        InvalidateRect(hwnd, NULL, FALSE);
                        return 0;
                    }
    ...
    case WM_LBUTTONUP: {
        ...
        if (g_fxRackUi.dragging && chain) {
            ...
            // Slot-0 compressor is pinned while the track has an external
            // sidechain source: it can't be removed or reordered away from 0.
            bool scPinned = (g_fxTrack >= 0 && g_fxTrack < MAX_TRACKS &&
                             g_Seq.trackSidechainSource[g_fxTrack] >= 0);
            g_fxRackUi.dragging = false;
            g_fxRackUi.dropSlot = -1;
            g_fxRackUi.overRemove = false;
            ...
            if (wasMoved) {
                ...
                if (fromChain && overLeft && src >= 0 && src < chain->count) {
                    // The pinned sidechain compressor can't be removed through
                    // the FX rack; it must be unlinked via the track's
                    // Sidechain Input context menu.
                    if (scPinned && src == 0) {
                        InvalidateRect(hwnd, NULL, FALSE);
                        return 0;
                    }
                    ...
                    if (!fromChain) {
                        ...
                        if (d && chain->count < FX_MAX_SLOTS) {
                            // Keep the pinned compressor at slot 0: force new
                            // modules from the browser to slot >= 1.
                            int insSlot = (scPinned && slot < 1) ? 1 : slot;
                            if (insSlot > chain->count) insSlot = chain->count;
                            push_undo_state();
                            seq_lock();
                            if (fx_chain_insert(chain, d, insSlot, (int)SAMPLE_RATE)) {
                                g_fxRackUi.selectedSlot = insSlot;
                                fx_rack_notify_main();
                            }
                            seq_unlock();
                        }
                    } else {
                        // Reordering the pinned slot-0 compressor away from 0 is
                        // disallowed; leave the chain untouched.
                        if (scPinned && src == 0) {
                            InvalidateRect(hwnd, NULL, FALSE);
                            return 0;
                        }
    ...
    case WM_RBUTTONDOWN: {
        ...
        if (chain && mx >= L.chainPane.left && mx <= L.chainPane.right &&
            ...
            int row = (my - L.chainPane.top - 2) / L.rowH;
            if (row >= 0 && row < chain->count) {
                // The pinned sidechain compressor can't be removed through the
                // FX rack; unlink it via the track's Sidechain Input menu.
                if (row == 0 && g_fxTrack >= 0 && g_fxTrack < MAX_TRACKS &&
                    g_Seq.trackSidechainSource[g_fxTrack] >= 0) {
                    InvalidateRect(hwnd, NULL, FALSE);
                    return 0;
                }
...
// Route `targetTrack` to duck against `sourceTrack`'s pre-FX signal, or -1 to
// clear the external sidechain. Reserves a compressor at FX slot 0 (pinned)
// with standard ducking defaults when a source is set.
static inline void set_track_sidechain_source(int targetTrack, int sourceTrack) {
    if (targetTrack < 0 || targetTrack >= MAX_TRACKS) return;
    if (sourceTrack < -1 || sourceTrack >= MAX_TRACKS) return;

    push_undo_state();
    seq_lock();

    g_Seq.trackSidechainSource[targetTrack] = (int8_t)sourceTrack;

    if (sourceTrack >= 0) {
        FxChain* chain = &g_TrackFx[targetTrack];
        bool slot0IsComp = (chain->count > 0 && chain->slots[0].desc &&
                            chain->slots[0].desc->type == FX_TYPE_COMPRESSOR);
        if (!slot0IsComp) {
            if (fx_chain_insert(chain, fx_descriptor_by_type(FX_TYPE_COMPRESSOR), 0, (int)SAMPLE_RATE)) {
                FxInstance* comp = &chain->slots[0];
                // Standard sidechain ducking defaults (Threshold/Ratio/Attack/
                // Release/Knee/Makeup).
                comp->params[0] = -20.0f; // Threshold dB
                comp->params[1] = 6.0f;   // Ratio :1
                comp->params[2] = 2.0f;   // Attack ms
                comp->params[3] = 80.0f;  // Release ms
                comp->params[4] = 4.0f;   // Knee dB
                comp->params[5] = 0.0f;   // Makeup dB
            }
        }
    } else {
        // Unlinking removes the reserved slot-0 compressor that was pinned for
        // sidechain ducking, leaving any other FX on the track untouched.
        FxChain* chain = &g_TrackFx[targetTrack];
        if (chain->count > 0 && chain->slots[0].desc &&
            chain->slots[0].desc->type == FX_TYPE_COMPRESSOR) {
            fx_chain_remove_at(chain, 0);
        }
    }
...
static inline void show_track_context_menu(HWND hwnd, int trackIdx, int screenX, int screenY) {
    ...
    // External sidechain routing submenu. Selecting a source track reserves a
    // compressor at FX slot 0 that ducks this track against that track's
    // signal; unlinking is done here too via the dedicated toggle below.
    {
        HMENU hScMenu = CreatePopupMenu();
        int curSc = g_Seq.trackSidechainSource[trackIdx];
        if (curSc >= 0) {
            AppendMenuA(hScMenu, MF_STRING, ID_TRACK_SC_UNLINK, "Unlink Sidechain");
            AppendMenuA(hScMenu, MF_SEPARATOR, 0, NULL);
        }
        AppendMenuA(hScMenu, MF_STRING | (curSc == -1 ? MF_CHECKED : 0), ID_TRACK_SC_NONE, "None (Internal)");
        AppendMenuA(hScMenu, MF_SEPARATOR, 0, NULL);
        for (int t = 0; t < g_Seq.trackCount; ++t) {
            if (t == trackIdx) continue; // Cannot sidechain a track to itself
            char tName[64];
            snprintf(tName, sizeof(tName), "Track %d", t + 1);
            UINT flags = MF_STRING | (curSc == t ? MF_CHECKED : 0);
            AppendMenuA(hScMenu, flags, ID_TRACK_SC_BASE + t, tName);
        }
        MENUINFO miSc = { sizeof(MENUINFO), MIM_STYLE, MNS_NOCHECK, 0, 0, 0, 0 };
        SetMenuInfo(hScMenu, &miSc);
        AppendMenuA(hMenu, MF_POPUP, (UINT_PTR)hScMenu, "Sidechain Input");
    }
    ...
    if (cmd == ID_TRACK_SC_NONE) {
        set_track_sidechain_source(trackIdx, -1);
        return;
    }
    if (cmd == ID_TRACK_SC_UNLINK) {
        set_track_sidechain_source(trackIdx, -1);
        return;
    }
    if (cmd >= ID_TRACK_SC_BASE && cmd < ID_TRACK_SC_BASE + MAX_TRACKS) {
        set_track_sidechain_source(trackIdx, cmd - ID_TRACK_SC_BASE);
        return;
    }
