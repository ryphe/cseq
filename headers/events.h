...
static inline LRESULT cseq_main_wndproc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_LBUTTONDOWN: {
            ...
            if (my > get_header_height()) {
                ...
                if (my >= btnY && my <= btnY + scale_y(30)) {
                    ...
                    int btnW = (availW - btnMargin * 2 - btnGap * 2) / 3;
                    ...
                    if (mx >= btnStartX + btnW + btnGap && mx <= btnStartX + btnW * 2 + btnGap && my <= btnY + scale_y(20)) {
                        seq_lock();
                        g_Seq.isDupSelectMode = !g_Seq.isDupSelectMode;
                        seq_unlock();
                        g_timelineDirty = true;
                        InvalidateRect(hwnd, NULL, FALSE);
                        return 0;
                    ...
                    if (mx >= btnStartX + btnW * 2 + btnGap * 2 && mx <= btnStartX + totalBtnsW && my <= btnY + scale_y(20)) {
                ...
                // Duplicate-select mode: a click over any track row duplicates
                // that track and exits the mode (so the click doesn't also mute
                // or deselect). Clicks outside a track row leave the mode armed.
                if (g_Seq.isDupSelectMode) {
                    int dupTrack = (my - get_header_height() + g_Seq.scrollY) / get_track_height();
                    if (dupTrack >= 0 && dupTrack < g_Seq.trackCount) {
                        g_Seq.lastClickedTrack = dupTrack;
                        duplicate_track_action(hwnd);
                        seq_lock();
                        g_Seq.isDupSelectMode = false;
                        seq_unlock();
                        g_timelineDirty = true;
                        InvalidateRect(hwnd, NULL, FALSE);
                        return 0;
    ...
    case WM_MOUSEMOVE: {
        ...
        // In dup-select mode the hovered track highlight must follow the cursor.
        if (g_Seq.isDupSelectMode) InvalidateRect(hwnd, NULL, FALSE);
