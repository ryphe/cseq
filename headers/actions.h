...
try_ogg:
     ...
     
    {
        ...
        if (opus_open(&d, filepath) && opus_decode_all(&d)) {
            seq_lock();
            if (g_Seq.sampleCount >= MAX_SAMPLES) {
                seq_unlock();
            ...
            baseName = baseName ? baseName + 1 : filepath;
            int idx = sample_finalize_loaded(filepath, d.pcm, d.frames, baseName);
            d.pcm = NULL;               // sample_finalize_loaded adopts ownership
            seq_unlock();
...
// Reorder a track (defined below); declared here because duplicate_track_action
// calls it to place the copy directly below the source track.
static inline void reorder_track(int from, int to);

// Duplicate a track: clone the active (last-clicked) track and append an exact
// copy at the end. Copies every per-track array, the per-track FX/granular
// engines (deep-copied so no heap pointers are shared), and all of the source
// track's clips. Synth clips are re-initialized off the lock (their transient
// renders are expensive and would starve the audio callback).
static inline void duplicate_track_action(HWND hwnd) {
    int src = g_Seq.lastClickedTrack;
    if (src < 0 || src >= g_Seq.trackCount) src = 0;

    // Count source clips so we can bail before mutating anything if there is
    // no room for the copy.
    int srcClipCount = 0;
    for (int i = 0; i < g_Seq.clipCount && i < MAX_CLIPS; ++i)
        if (g_Seq.clips[i].track == src) srcClipCount++;

    if (g_Seq.trackCount >= MAX_TRACKS) return;
    if (g_Seq.clipCount + srcClipCount > MAX_CLIPS) return;

    seq_lock();
    int newIdx = g_Seq.trackCount;

    // --- Copy per-track arrays (value structs). ---
    g_Seq.trackMuted[newIdx] = g_Seq.trackMuted[src];
    g_Seq.trackSolo[newIdx] = g_Seq.trackSolo[src];
    g_Seq.trackVolume[newIdx] = g_Seq.trackVolume[src];
    g_Seq.trackPan[newIdx] = g_Seq.trackPan[src];
    g_Seq.trackWidth[newIdx] = g_Seq.trackWidth[src];
    g_Seq.trackThemes[newIdx] = g_Seq.trackThemes[src];
    g_Seq.trackTriggerProb[newIdx] = g_Seq.trackTriggerProb[src];
    g_Seq.trackRngState[newIdx] = g_Seq.trackRngState[src];

    g_Seq.trackEQ[newIdx] = g_Seq.trackEQ[src];
    g_Seq.trackEqLow[newIdx] = g_Seq.trackEqLow[src];
    g_Seq.trackEqMid[newIdx] = g_Seq.trackEqMid[src];
    g_Seq.trackEqHigh[newIdx] = g_Seq.trackEqHigh[src];
    g_Seq.trackEqActive[newIdx] = g_Seq.trackEqActive[src];
    for (int k = 0; k < 3; ++k) {
        g_Seq.trackPeak[newIdx][k] = g_Seq.trackPeak[src][k];
        g_Seq.trackEqFreq[newIdx][k] = g_Seq.trackEqFreq[src][k];
        g_Seq.trackEqQ[newIdx][k] = g_Seq.trackEqQ[src][k];
    }

    g_Seq.trackFilter[newIdx] = g_Seq.trackFilter[src];
    g_Seq.trackUI[newIdx] = g_Seq.trackUI[src];
    // trackAudio is audio-thread runtime state (holds pointers); leave the new
    // track's fresh slot alone, the audio thread rebuilds it.

    // --- Deep-copy the FX chain (re-inits fresh per-instance state). ---
    fx_chain_to_snapshot(&g_TrackFx[newIdx], &g_TrackFx[src]);

    // --- Copy the per-track granular engine, dropping owned-sample pointers. ---
    {
        GranularEngine tmp = g_TrackGran[src];
        tmp.ownFrames = NULL;
        tmp.ownFrameCount = 0;
        tmp.ownLoaded = false;
        memset(&tmp.ownSample, 0, sizeof(AudioSample));
        tmp.trackIdx = newIdx;
        tmp.clipIdx = -1;
        g_TrackGran[newIdx] = tmp;
    }

    // --- Duplicate all clips on the source track. ---
    int synthCloneIdx[MAX_CLIPS];
    int synthCloneCount = 0;
    for (int i = 0; i < g_Seq.clipCount && i < MAX_CLIPS; ++i) {
        if (g_Seq.clips[i].track != src) continue;
        int cloneIdx = g_Seq.clipCount++;
        g_Seq.clips[cloneIdx] = g_Seq.clips[i];
        g_Seq.clips[cloneIdx].track = newIdx;
        g_Seq.clips[cloneIdx].isSelected = false;

        g_ClipGran[cloneIdx] = g_ClipGran[i];
        g_ClipGran[cloneIdx].clipIdx = cloneIdx;
        g_ClipGran[cloneIdx].trackIdx = newIdx;
        g_ClipGran[cloneIdx].ownFrames = NULL;
        g_ClipGran[cloneIdx].ownFrameCount = 0;
        g_ClipGran[cloneIdx].ownLoaded = false;
        memset(&g_ClipGran[cloneIdx].ownSample, 0, sizeof(AudioSample));
    ...
    // Re-init synth runtime state off the lock (Quadrum transient render).
    ...
    // Move the just-appended duplicate so it sits directly below the source.
    if (src + 1 < g_Seq.trackCount)
        reorder_track(g_Seq.trackCount - 1, src + 1);
}
...
#define SEQ_SWAP_TRACK_ARRAY(type, arr) \
    ...
    SEQ_SWAP_TRACK_ARRAY(float, g_Seq.trackTriggerProb);
    SEQ_SWAP_TRACK_ARRAY(uint32_t, g_Seq.trackRngState);
