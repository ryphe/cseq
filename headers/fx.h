...
struct FxInstance {
    ...
    void* state;                        
    float scFeedL, scFeedR;             // external sidechain feed (compressor slot 0 only)
    bool  scActive;                     // set per-frame by the render engine
};
...
static void fx_buff_process(FxInstance* fx, float* L, float* R) {
    ...
    if (fabsf(rate) < 0.01f) rate = (rate < 0.0f) ? -0.01f : 0.01f;
    ...
    else if (s->readPos < 0.0f)    s->readPos += (float)size;
...
static void fx_compressor_process(FxInstance* fx, float* L, float* R) {
    ...
    // envelope. When an external sidechain feed is active (slot-0 compressor
    // routed to another track), the detector watches that feed instead of the
    // track's own signal, ducking the input against it.
    float detL = inL, detR = inR;
    if (fx->scActive) { detL = fx->scFeedL; detR = fx->scFeedR; }
    float absL = fabsf(detL);
...
static const FxParamDef kFxBuffParams[] = {
    ...
    { "Rate", FX_PARAM_KNOB, -2.0f, 2.0f, 0.0f, "%.2fx" },
...
static inline void fx_instance_init(FxInstance* fx, int sr) {
    fx->scFeedL = 0.0f; fx->scFeedR = 0.0f; fx->scActive = false;
