...
| **MIDI & Sequencing** | Polyphonic piano roll with velocity, length, and pitch editing. Built‑in **generative note sequencer** (root, scale, chord, arpeggiator patterns, direction, octave range, seed). **Humanizer** for timing, duration, and velocity jitter. Per‑track **trigger probability** (0–100%) for random playback |
...
| **Modular FX Rack** | Drag-and-drop rack hosting up to 12 serial effect slots per track across 12 modules:<br>• **Autopan**: Cyclic stereo field panner with adjustable rate, depth, and width.<br>• **Buff**: Real‑time audio buffer for glitch, loop, and stutter repeats.<br>• **Chorus**: Multi-voice stereo doubler and wide BBD-style ensemble chorus.<br>• **Compressor**: Stereo-linked VCA feed-forward compressor (threshold, ratio, attack, release).<br>• **Delay**: Tape-style delay with ping-pong stereo mode, filtering, and saturation.<br>• **Flanger**: Short comb-filtering delay with feedback for classic sweeping jet modulation.<br>• **Gain**: Clean output level trim and boost/cut.<br>• **Lofi**: Digital degradation with variable bit-crushing and sample-rate decimation.<br>• **Phaser**: 6-stage allpass network driven by a quadrature sweep LFO.<br>• **Resonator**: Zero-delay feedback (ZDF) SVF tuned to musical pitches for ringing tones.<br>• **Reverb**: Freeverb/tank network with adjustable predelay, damping, and diffusion.<br>• **Tremolo**: Amplitude modulation with variable rate, depth, and shaping. |
| **Sidechain Compression** | Track-level context menu allows live external sidechaining with dedicated compressor FX slot
...
| **Quadrum Drum Synth** | 8‑voice procedural virtual‑analog drum synthesizer (Kick, Snare, Clap, Closed Hat, Open Hat, Tom, Cowbell, Cymbal). Dynamic pitch sweeps, FM, noise shaping, transient clicks, biquad filtering, saturation, multi‑tap clap burst/flam, and velocity‑sensitive trigger |
