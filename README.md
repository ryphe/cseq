...
| **Track Headers** | Click the track number/name area to **mute** the track; `Shift+Drag` vertically to **reorder** tracks. Hover and use the **scroll wheel** to adjust track volume quickly. Right‑click to open dedicated dialogs for **Parametric EQ**, **FX Rack**, **Pan & Stereo Width**, **Filter Plotter**, and **Granular Engine** |
| **Piano Roll** | Click on the note grid to **add** a note; double‑click or right‑click to **delete**. Drag a note left/right to **move** it in time; drag its right edge to **resize** its length. Scroll the mouse wheel while hovering over a note to adjust its **velocity** (0–127). The four ADSR knobs (Attack, Decay, Sustain, Release) shape the amplitude envelope of every note in the clip. Right‑click for per‑note context options |
...
| **SoundFont 2 Player** | Load `.sf2` banks with full preset browser and instrument selector. Pre‑rendered note cache ensures zero‑latency, deterministic playback — no real‑time synthesis lock‑in, just sample‑accurate streaming |
...
| **Visualizer** | Real‑time spectrum + oscilloscope overlay (OSC/SPEC/COMBO modes). Adjustable FFT size (256–8192), channel routing (Stereo / L / R / Mid‑Side), zoom, and colour hue. Hover for frequency/note/dB tooltips. Driven by a lock‑free audio ring — zero impact on the real‑time path |
...
| **Real‑Time Core** | Audio callback is strictly allocation‑free, I/O‑free, and GUI‑free. SIMD‑accelerated track summing (AVX2/SSE2). Denormal‑hardened (FTZ/DAZ) for stable CPU usage. Heavy lifting (decode, save, export, peak‑building, SF2 pre‑render) runs on background workers — playback never stalls |
