...
| **FX Rack** | Modular chain hosting up to 8 serial effects per track. Drag any of the 12 modules from the left pool into the chain; drag vertically to reorder; right-click to remove. Click a slot to reveal bottom-strip parameter knobs and sliders with mouse wheel adjustment |
...
| **Modular FX Rack** | Drag-and-drop rack hosting up to 8 serial effect slots per track across 12 modules:<br>• **Gain** (`output/gain`): Clean output level trim and boost/cut.<br>• **Buff** (`buffer/glitch`): Real‑time audio buffer for glitch, loop, and stutter repeats.<br>• **Delay** (`tape/echo`): Tape-style delay with ping-pong stereo mode, filtering, and saturation.<br>• **Reverb** (`freeverb/tank`): Freeverb/tank network with adjustable predelay, damping, and diffusion.<br>• **Lofi** (`crush/decimate`): Digital degradation with variable bit-crushing and sample-rate decimation.<br>• **Phaser** (`6-stage/sweep`): 6-stage allpass network driven by a quadrature sweep LFO.<br>• **Chorus** (`doubler/wide`): Multi-voice stereo doubler and wide BBD-style ensemble chorus.<br>• **Compressor** (`VCA/glue`): Stereo-linked VCA feed-forward compressor (threshold, ratio, attack, release).<br>• **Resonator** (`ring/tone`): Zero-delay feedback (ZDF) SVF tuned to musical pitches for ringing tones.<br>• **Tremolo** (`pulse/vibe`): Amplitude modulation with variable rate, depth, and pulse shaping.<br>• **Autopan** (`stereo/pan`): Cyclic stereo field panner with adjustable rate, depth, and width.<br>• **Flanger** (`jet/flange`): Short comb-filtering delay with feedback for classic sweeping jet modulation. |
