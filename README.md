...
| **Mixing & FX** | Per‑track **3‑band parametric EQ** (sweepable frequency/Q, `±`12dB`). **Stackable filter plotter** (LP/HP/BP/Notch) with real‑time drag. **Modular FX rack**: Gain, Buff, Delay, Reverb, Compressor, Phaser, Chorus, Resonator, Lofi. Master **limiter/clipper** (switchable) and **Lo‑Fi** bus (bit depth `8‑12`, sample rate `8‑32 kHz`) |
| **UI & Workflow** | 120 FPS GDI pipeline with FNV hash‑tree dirty tracking for partial redraws. Content‑addressed PCM disk cache (FNV‑1a) deduplicates decoded audio. **Media Explorer** with loopable preview. Full **Undo/Redo** with atomic snapshots in a circular buffer |
| **Project & Export** | Versioned trailing sections ensure forward/backward compatibility. Deterministic `WAV` export (16/24/32‑bit with TPDF dithering) using the exact same render loop as live playback. Output is bit‑for‑bit identical to what you hear |
